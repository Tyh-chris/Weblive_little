<template>
    <div class="live-list">
        <el-dialog title="直播" :visible.sync="showEditLive">
            <el-form v-model="form" label-width="100px">
               ...
               <el-form-item label="简介">
                    <el-input v-model="form.desc" placeholder="输入简介"/>
                </el-form-item>
            </el-form>
        ...
        <div class="live-top" v-show="showType">
            <div class="text-box">
        ...
        <div class="live-container">
            <div class="list-box" v-if="liveRoom.length !==0">
                <div class="live-item" :key=index v-for="(item, index) in liveRoom">
                    <div class="item-box" @click="playerHandler(item.roomId, item.anchorId,item.roomTitle,item.roomDesc)">
                        <p>
                            <img class="live-img"  :src="item.url"/>
                            <el-tooltip v-if="!isMobile" class="item" effect="dark" :content="item.roomDesc" placement="top-start">
                                <span class="live-name">{{item.roomTitle || item.roomId}}</span>
                            </el-tooltip>
                            <!-- <span  v-else class="live-name">{{item.roomTitle || item.roomId}}</span> -->
...
<script>
  ...
  import img2 from  '../../assets/image/live-img/2.png'
  import img3 from  '../../assets/image/live-img/img-3.png'
  import img4 from  '../../assets/image/live-img/img-4.png'
  import img5 from  '../../assets/image/live-img/5.png'
  import img6 from  '../../assets/image/live-img/6.png'
  export default {
    name: 'liveList',
    components: {
      Login
    ...
    computed: {
      ...mapState({
        ...
        showType:state => state.userType==0?true:false,
        // isLoginText:state => state.user.isLogin==false?("登录"):("我要直播")
      }),
    },
    created() {
      this._stat()
    ...
    methods: {
        ...
        axios(
            {
                ...
                method:'post',
                url:this.$store.state.baseUrl+'/api/room/creatRoom',
                params:{roomTitle:title,roomDesc:desc},
            }
        ).then((res)=>{
            // console.log(res)
            if(res.data.code == 200){
                console.log(res.data.data.roomId);
                this.$store.commit('setGroupId', res.data.data.roomId)
                this.$router.push('/pusher')
                // console.log(this.$store.setGroupId)
            }else{
                this.$message({
        ...
        axios(
            {
                // headers:{token:this.$store.state.token},
                method:'get',
                url:this.$store.state.baseUrl+'/api/live/getPlayerList',
                // params:{roomTitle:title,roomDesc:desc},
            }
        ).then((res)=>{
            console.log(res)
            this.liveRoom = res.data.data.playerList
            this.liveRoom.forEach((item)=> {
              item.url = this.img_list[this.getRandomInt(0, 5)]
              item.roomDesc='简介：'+item.roomDesc
            })
          })
          .catch(() => {
            this.$store.commit('showMessage', { message: '失败', type: 'error' })
      ...
      playerHandler(roomId, anchorId, title,desc) {
